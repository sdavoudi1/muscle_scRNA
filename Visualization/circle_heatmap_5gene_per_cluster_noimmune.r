...
# Young_noimmune_v2
# After merging FAP_1 and FAP_3 from young_noimmune
...
# Then we assign cluster order and names
cluster_order <- c(2, 5, 3, 1, 4, 7, 8, 6)
cluster_names <- c("EC_1", "EC_2", "MuSC", "FAP_1", "FAP_2", "FAP_3", "Schwann", "Tenocyte")

# Then we use the circle_heatmap function to draw the graph.
young_noimmune_v2_circle_heatmap_5genes <- circle_heatmap(dataset = young_noimmune_v2, diff_markers = young_noimmune_v2_diff_markers,
							image_name = "young_noimmune_v2_circle_heatmap.png", image_dpi = 600, 
							image_width = 10, image_height = 5, n_genes = 5, cluster_names = cluster_names, 
							cluster_order = cluster_order)
