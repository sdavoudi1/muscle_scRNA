...
genes <- c("Cd79a", "Cd68", "Pecam1", "Pax7", "Myod1", "Des","Col1a1", "Ly6a", "Pdgfra", "Tnmd")

# Next we get the average expression per cluster for the genes of interest:
avg_exp <- AverageExpression(aged, genes.use = genes, show.progress = F)

# Next we reorder the columns in the avg_exp dataframe to be in the order that we want
avg_exp <- avg_exp[c(7,3,10,1,4,6,2,5,8,9)]

# Next we rename the cluster numbers to the cell types
names(avg_exp) <- c("Immune_Cells", "EC", "MuSCs", "Myogenic_1", "Myogenic_2", "Myogenic_3", "FAP_1", "FAP_2", "FAP_3","Tenocytes")
